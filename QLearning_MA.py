...
for ep in range(1, 6):
    action_frequency = {i: 0 for i in range(3)}
    decision_action = {'Exploitation': 0, 'Exploration': 0}
    agv_reward_table = {i: [] for i in range(params['learner_population'])}
    env.reset()
    ...
    for tick in range(400):
        ...
        for agent in env.agent_iter(max_iter=params['learner_population']):
            ...
            if random.uniform(0, 1) < epsilon:
                ...
                decision_action['Exploitation'] += 1
    ...
    print('Exploitation times: ', decision_action['Exploitation'])
    print('Exploration times: ', decision_action['Exploration'])
    for i in action_frequency:
        d[i].append(action_frequency[i])
    epsilon *= decay
...
'''
episodes = [1, 2, 3, 4, 5]
plt.plot(episodes, d[0], label='Random Walk' color='red')
plt.plot(episodes, d[1], label='lay pheromone', color='blue')
plt.plot(episodes, d[2], label='follow pheromone', color='green')
plt.xlabel('EPISODES')
plt.ylabel('FREQUENCY')
plt.set_title('Action Frequency')
plt.show()
'''
